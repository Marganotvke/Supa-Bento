...
function _inlineListElements(idx, item, theme, anim){
    ...
    return <a href={item.link} key={idx} 
        ...
        className={`w-[80%] max-w-full p-2 min-w-0 items-center justify-center inline-flex overflow-hidden text-ellipsis whitespace-nowrap hover:text-[--hoverText] hover:bg-[--hover] ${theme.text.isBold ? "font-semibold" : ""}`}>
...
function _listItem(idx, itemList, config, anim){
    ...
    return <div key={idx} className={`flex flex-col min-w-0 drop-shadow-lg hover:-translate-y-1`} style={{borderRadius: theme.borderRadius, backgroundColor: theme.app, ...anim}}>
        ...
        className="flex flex-1 flex-col min-w-0 items-center justify-center text-[--text] p-2">
...
export default function ListBox({idx, config ,isHidden}){
    ...
    return <>
        <div style={{gridTemplateColumns: `repeat(${listConf.layout.cols}, minmax(0, 1fr))`, gap: `${listConf.layout.gap}rem`}}
            className={`md:grid ${isHidden? "hidden" : "grid"} min-w-0 p-2 text-[--text] hover:text-[--hoverText]`} >
